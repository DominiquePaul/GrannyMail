...
class TestWhatsappMessenger:
    ...
    @pytest.mark.asyncio
    @patch("uuid.uuid4", new_callable=Mock)
    async def test_process_message_interactive(self, mock_uuid4, fake_uow, wa_message):
        # setup
    ...
    def test_get_or_create_user__create(self, fake_uow):
        whatsapp = Whatsapp()
        timestamp = utils.get_utc_timestamp()
        user_returned = whatsapp._get_or_create_user(fake_uow, "44131231231", timestamp)
    ...
    async def test_reply_text(self, mock_post_httpx, fake_uow, wa_message):
